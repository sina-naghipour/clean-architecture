...
class TestProductServiceRepositoryIntegration:
    ...
    def test_create_product_flow(self, service, mock_request, repository):
        ...
        result = service.create_product(mock_request, product_data)
        
        assert result.status_code == 201
        created_id = result.headers["Location"].split("/")[-1]
        
        retrieved = repository.get_product_by_id(created_id)
        assert retrieved is not None
        assert retrieved.name == product_data.name
        assert retrieved.price == 99.99

    def test_get_product_flow(self, service, mock_request, repository):
        ...
        created = repository.create_product(product_db)
        
        result = service.get_product(mock_request, created.id)
        
        assert result.id == created.id
        assert result.name == product_db.name
        assert result.price == 49.99

    def test_list_products_flow(self, service, mock_request, repository):
        for i in range(3):
            product = ProductDB(
    ...
    def test_update_product_flow(self, service, mock_request, repository):
        original = ProductDB(
            name=f"Original Name {id(self)}",
            price=25.0,
            stock=30
        )
        created = repository.create_product(original)
        ...
        result = service.update_product(mock_request, created.id, update_data)
        
        assert result.name == update_data.name
        assert result.price == 35.0
        
        updated_in_db = repository.get_product_by_id(created.id)
        assert updated_in_db.name == update_data.name
        assert updated_in_db.price == 35.0

    def test_patch_product_flow(self, service, mock_request, repository):
        original = ProductDB(
        ...
        result = service.patch_product(mock_request, created.id, patch_data)
        
        assert result.stock == 75
        assert result.name == original.name
        
        patched_in_db = repository.get_product_by_id(created.id)
        assert patched_in_db.stock == 75

    def test_delete_product_flow(self, service, mock_request, repository):
        product = ProductDB(
        ...
        result = service.delete_product(mock_request, created.id)
        
        assert result is None
        
        deleted = repository.get_product_by_id(created.id)
        assert deleted is None

    def test_update_inventory_flow(self, service, mock_request, repository):
        product = ProductDB(
        ...
        result = service.update_inventory(mock_request, created.id, inventory_data)
        
        assert result.stock == 50
        
        updated = repository.get_product_by_id(created.id)
        assert updated.stock == 50

    def test_error_propagation_from_repository(self, service, mock_request, repository):
    ...
    def test_search_flow(self, service, mock_request, repository):
